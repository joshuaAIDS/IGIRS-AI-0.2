...
export default function Globe({ isSpeaking = false, state = "standby" }: GlobeProps) {
  ...
  useEffect(() => {
   ...
   `
    ...
    const animate = () => {
      ...
      // Dynamic vocal animation when speaking
      const isVoiceActive = isSpeakingRef.current || stateRef.current === "speaking"
      let voiceScale = 1.0
      if (isVoiceActive) {
        voiceScale = 1.0 + Math.sin(Date.now() * 0.007) * 0.065 + Math.cos(Date.now() * 0.016) * 0.03
        wireframeGlobe.rotation.y += 0.006
        solidGlobe.rotation.y += 0.006
        atmosphereMesh.rotation.y += 0.002
      } else if (stateRef.current === "thinking") {
        voiceScale = 1.0 + Math.sin(Date.now() * 0.01) * 0.04
        wireframeGlobe.rotation.y += 0.012
        solidGlobe.rotation.y += 0.012
      } else {
      ...
      wireframeGlobe.scale.set(voiceScale, voiceScale, voiceScale)
      solidGlobe.scale.set(voiceScale, voiceScale, voiceScale)
      atmosphereMesh.scale.set(voiceScale, voiceScale, voiceScale)
    ...
    const loadTexture = (url: string) =>
      new Promise((resolve) => {
        textureLoader.load(
          ...
          undefined,
          () => resolve(null) // Handle graceful fallback if offline
        )
      })
    ...
    Promise.all([
      ...
      if (!texture) return // Keep procedural glow if texture files not present
